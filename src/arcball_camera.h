...
class ArcBallCamera {
public:
  ArcBallCamera() : last_mouse_x_(0.0f), last_mouse_y_(0.0f), pitch_(0.0f), yaw_(0.0f), dolly_(0.0f), tx_(0.0f), ty_(0.0f) {}

  inline
  void event(bool const bMoving, bool const btnTranslate, bool const btnRotate,
             float const mouseX, float const mouseY, float const wheelData) {
    if (bMoving) {
  ...
  inline
  void eventMouseMoved(bool const btnTranslate, bool const btnRotate, float const mouseX, float const mouseY) {
